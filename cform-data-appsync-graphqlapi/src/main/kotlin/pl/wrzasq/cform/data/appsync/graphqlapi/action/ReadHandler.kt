...
package pl.wrzasq.cform.data.appsync.graphqlapi.action
...
import pl.wrzasq.cform.data.appsync.graphqlapi.model.fromReadResponse
import pl.wrzasq.cform.data.appsync.graphqlapi.model.toReadRequest
...
class ReadHandler(
    ...
    override fun handleRequest(
        ...
        return proxy.initiate(
            "WrzasqPl-AppSync-GraphQlApiData::Read",
            proxyClient,
            ...
            .makeServiceCall { awsRequest, client ->
                try {
                    client.injectCredentialsAndInvokeV2(
                        awsRequest,
                        client.client()::getGraphqlApi
                    ).also {
                        logger.log("${ResourceModel.TYPE_NAME} has successfully been read.")
                    }
                } catch (error: NotFoundException) {
                    throw CfnNotFoundException(ResourceModel.TYPE_NAME, awsRequest.apiId(), error)
